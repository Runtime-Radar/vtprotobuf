...
```
import "github.com/runtime-radar/vtprotobuf/vtproto/ext.proto";
...
1. Install `protoc-gen-go-vtproto`:
    ...
    ```
    go install github.com/runtime-radar/vtprotobuf/cmd/protoc-gen-go-vtproto@latest
    ```
...
To use `vtprotobuf` with the new versions of GRPC, you need to register the codec provided by the `github.com/runtime-radar/vtprotobuf/codec/grpc` package.

```go
package servenv

import (
	"github.com/runtime-radar/vtprotobuf/codec/grpc"
...
To use `vtprotobuf` as a DRPC encoding, simply pass `github.com/runtime-radar/vtprotobuf/codec/drpc` as the `protolib` flag in your `protoc-gen-go-drpc` invocation.

Example:

```
protoc --go_out=. --go-vtproto_out=. --go-drpc_out=. --go-drpc_opt=protolib=github.com/runtime-radar/vtprotobuf/codec/drpc
```
